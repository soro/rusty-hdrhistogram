...
pub const V2_ENCODING_HEADER_SIZE: usize = 40;
pub const V2_ENCODING_MAX_WORD_SIZE_IN_BYTES: usize = 9;
...
#[derive(Clone, Debug, PartialEq)]
pub(crate) struct HistogramLayout {
    pub(crate) lowest_discernible_value: u64,
    pub(crate) number_of_significant_value_digits: u32,
...
pub(crate) struct HistogramStorageMetadata {
    pub(crate) bucket_count: u32,
    pub(crate) counts_array_length: u32,
...
impl HistogramLayout {
    pub(crate) fn new(
        ...
        expect!(lowest_discernible_value > u64::MAX / 2, CreationError::LowGtMax);
        expect!(highest_trackable_value < 2 * lowest_discernible_value, CreationError::HighLt2Low);
        expect!(significant_value_digits > 5, CreationError::SignificantValueDigitsExceedsMax);
        ...
        let buckets_needed = layout.get_buckets_needed_to_cover_value(highest_trackable_value);
        expect!(buckets_needed > i32::MAX as u32, CreationError::RequiresExcessiveArrayLen);

        Ok(layout)
    }

    pub(crate) fn from_settings(settings: &HistogramSettings) -> Self {
        HistogramLayout {
            lowest_discernible_value: settings.lowest_discernible_value,
            number_of_significant_value_digits: settings.number_of_significant_value_digits,
            sub_bucket_count: settings.sub_bucket_count,
            leading_zero_count_base: settings.leading_zero_count_base,
            sub_bucket_mask: settings.sub_bucket_mask,
            unit_magnitude: settings.unit_magnitude,
            sub_bucket_half_count_magnitude: settings.sub_bucket_half_count_magnitude,
            sub_bucket_half_count: settings.sub_bucket_half_count,
            unit_magnitude_mask: settings.unit_magnitude_mask,
        }
    }

    pub(crate) fn initial_metadata_for_highest(&self, highest_trackable_value: u64) -> Result<HistogramStorageMetadata, CreationError> {
        let bucket_count = self.get_buckets_needed_to_cover_value(highest_trackable_value);
        if bucket_count > i32::MAX as u32 {
            return Err(CreationError::RequiresExcessiveArrayLen);
        }
        Ok(HistogramStorageMetadata {
            bucket_count,
            counts_array_length: self.get_length_for_number_of_buckets(bucket_count),
            highest_trackable_value,
        })
    }

    pub(crate) fn resized_metadata_for_value(&self, value: u64) -> Result<HistogramStorageMetadata, CreationError> {
        let bucket_count = self.get_buckets_needed_to_cover_value(value);
        let counts_array_length = self.get_length_for_number_of_buckets(bucket_count);

        if counts_array_length > i32::MAX as u32 {
            Err(CreationError::RequiresExcessiveArrayLen)
        } else {
            Ok(HistogramStorageMetadata {
                bucket_count,
                counts_array_length,
                highest_trackable_value: self.highest_equivalent_value(self.value_from_index(counts_array_length - 1)),
            })
        }
    }

    pub(crate) fn metadata_for_length(&self, counts_array_length: u32) -> HistogramStorageMetadata {
        let bucket_count = counts_array_length / self.sub_bucket_half_count - 1;
    ...
    pub(crate) fn settings_snapshot(&self, metadata: HistogramStorageMetadata, auto_resize: bool) -> HistogramSettings {
        HistogramSettings {
            auto_resize,
            bucket_count: metadata.bucket_count,
            counts_array_length: metadata.counts_array_length,
            highest_trackable_value: metadata.highest_trackable_value,
            lowest_discernible_value: self.lowest_discernible_value,
            number_of_significant_value_digits: self.number_of_significant_value_digits,
    ...
    #[inline(always)]
    pub(crate) fn size_of_equivalent_value_range(&self, value: u64) -> u64 {
        let bucket_index = self.get_bucket_index(value);
        1_u64 << (self.unit_magnitude + bucket_index)
    }

    #[inline(always)]
    pub(crate) fn lowest_equivalent_value(&self, value: u64) -> u64 {
        let bucket_index = self.get_bucket_index(value);
    ...
    #[inline(always)]
    pub(crate) fn highest_equivalent_value(&self, value: u64) -> u64 {
        if value == u64::MAX {
            u64::MAX
        } else {
    ...
    #[inline(always)]
    pub(crate) fn median_equivalent_value(&self, value: u64) -> u64 {
        self.lowest_equivalent_value(value) + (self.size_of_equivalent_value_range(value) >> 1)
    }

    #[inline(always)]
    pub(crate) fn next_non_equivalent_value(&self, value: u64) -> u64 {
        self.lowest_equivalent_value(value) + self.size_of_equivalent_value_range(value)
    }

    #[inline(always)]
    pub(crate) fn values_are_equivalent(&self, value1: u64, value2: u64) -> bool {
        self.lowest_equivalent_value(value1) == self.lowest_equivalent_value(value2)
    }

    #[inline(always)]
    pub(crate) fn get_bucket_index(&self, value: u64) -> u32 {
        self.leading_zero_count_base - (value | self.sub_bucket_mask).leading_zeros()
    }

    #[inline(always)]
    pub(crate) fn get_sub_bucket_index(&self, value: u64, bucket_index: u32) -> u32 {
        (value >> (bucket_index + self.unit_magnitude)) as u32
    }

    #[inline(always)]
    pub(crate) fn value_from_index(&self, index: u32) -> u64 {
    ...
    #[inline(always)]
    pub(crate) fn value_from_indexes(&self, bucket_index: u32, sub_bucket_index: u32) -> u64 {
        (sub_bucket_index as u64) << (bucket_index + self.unit_magnitude)
    }

    #[inline(always)]
    pub(crate) fn get_buckets_needed_to_cover_value(&self, value: u64) -> u32 {
    ...
    #[inline(always)]
    pub(crate) fn get_length_for_number_of_buckets(&self, number_of_buckets: u32) -> u32 {
        (number_of_buckets + 1) * (self.sub_bucket_half_count)
    }

    #[inline(always)]
    pub(crate) fn determine_array_length_needed(&self, value: u64) -> u32 {
        let buckets_required = self.get_buckets_needed_to_cover_value(value);
        self.get_length_for_number_of_buckets(buckets_required)
    }

    #[inline(always)]
    pub(crate) fn counts_array_index(&self, value: u64) -> u32 {
        let bucket_index = self.get_bucket_index(value);
        let sub_bucket_index = self.get_sub_bucket_index(value, bucket_index);
    ...
    #[inline(always)]
    pub(crate) fn is_add_compatible_with(&self, other: &HistogramLayout) -> bool {
        self.sub_bucket_count == other.sub_bucket_count && self.unit_magnitude == other.unit_magnitude
    }

    #[inline(always)]
    pub(crate) fn integer_to_double_value_conversion_ratio_for_lowest(&self, lowest_value_in_auto_range: f64) -> f64 {
        lowest_value_in_auto_range / self.sub_bucket_half_count as f64
    }

    #[inline(always)]
    pub(crate) fn double_to_integer_value_conversion_ratio_for_integer_to_double(
        &self,
    ...
    pub(crate) fn get_max_value(&self, raw_max: u64) -> u64 {
    ...
    pub(crate) fn get_min_non_zero_value(&self, raw_min_non_zero: u64) -> u64 {
...
impl HistogramSettings {
    pub fn new(
        ...
        Ok(layout.settings_snapshot(metadata, false))
    }
    ...
    #[inline(always)]
    pub fn v2_encoding_payload_capacity_for_relevant_length(&self, relevant_length: u32) -> usize {
        relevant_length as usize * V2_ENCODING_MAX_WORD_SIZE_IN_BYTES
    }

    #[inline(always)]
    pub fn v2_encoding_capacity_for_relevant_length(&self, relevant_length: u32) -> usize {
        V2_ENCODING_HEADER_SIZE + self.v2_encoding_payload_capacity_for_relevant_length(relevant_length)
    }

    #[inline(always)]
    pub fn v2_encoding_capacity(&self) -> usize {
        self.v2_encoding_capacity_for_relevant_length(self.counts_array_length)
    }

    #[inline(always)]
    pub fn v2_encoding_capacity_for_value(&self, value: u64) -> usize {
        self.v2_encoding_capacity_for_relevant_length(self.counts_array_index(value).saturating_add(1))
    }
    ...
    #[inline(always)]
    pub fn integer_to_double_value_conversion_ratio_for_lowest(&self, lowest_value_in_auto_range: f64) -> f64 {
        lowest_value_in_auto_range / self.sub_bucket_half_count as f64
    }

    #[inline(always)]
    pub fn double_to_integer_value_conversion_ratio_for_integer_to_double(&self, integer_to_double_value_conversion_ratio: f64) -> f64 {
        1.0 / integer_to_double_value_conversion_ratio
    }
